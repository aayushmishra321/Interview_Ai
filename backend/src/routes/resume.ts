...
router.get('/:id/view', asyncHandler(async (req, res) => {
  ...
  try {
    ...
    // If resume has a file URL
    if (resume.fileUrl) {
      logger.info(`Viewing resume: ${id} for user ${req.user!.userId}`);
      
      // For Cloudinary URLs, modify to force inline display (not download)
      if (resume.fileUrl.includes('cloudinary.com')) {
        // Add fl_attachment:false to force inline viewing
        let viewUrl = resume.fileUrl;
        
        // If URL already has parameters, append; otherwise add
        if (viewUrl.includes('?')) {
          viewUrl += '&fl_attachment=false';
        } else {
          // For Cloudinary, we need to add the flag in the transformation part
      ...
      // For local files, serve with inline content-disposition
      if (resume.localFilePath) {
        return res.sendFile(resume.localFilePath, {
          headers: {
...
router.get('/:id/download', asyncHandler(async (req, res) => {
  ...
  try {
    ...
    // If resume has a file URL, redirect to it
    if (resume.fileUrl) {
      ...
      // For local files, serve the file with download header
      if (resume.localFilePath) {
        return res.download(resume.localFilePath, resume.filename);
      }
