...
export const adminService = {
  ...
  async getSystemMetrics(): Promise<SystemMetrics> {
    const response = await api.get<SystemMetrics>('/api/admin/system-metrics');
    return response.data as SystemMetrics;
  },

  async getAIMetrics(): Promise<AIMetrics> {
    const response = await api.get<AIMetrics>('/api/admin/ai-metrics');
    return response.data as AIMetrics;
  },

  async getActivity(limit: number = 20): Promise<Activity[]> {
    const response = await api.get<Activity[]>(`/api/admin/activity?limit=${limit}`);
