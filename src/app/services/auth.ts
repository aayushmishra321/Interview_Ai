...
class AuthService {
  ...
  async refreshToken(): Promise<APIResponse<AuthTokens>> {
    ...
    const response = await apiService.post<AuthTokens>('/api/auth/refresh', {
      refreshToken,
    });
  ...
  async verifyOTP(email: string, otp: string): Promise<APIResponse<{ verified: boolean }>> {
    return apiService.post('/api/auth/verify-otp', { email, otp });
  }
  ...
  async handleOAuthCallback(code: string): Promise<APIResponse<LoginResponse>> {
    const response = await apiService.post<LoginResponse>('/api/auth/google/callback', { code });
